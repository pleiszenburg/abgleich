...
pub struct TransferIncrementalBuilder<'a> {
    source: &'a Location,
    target: &'a Location,
    dataset: String,
...
impl<'a> TransferIncrementalBuilder<'a> {
    #[must_use]
    pub const fn new(
        source: &'a Location,
        target: &'a Location,
        ...
        Self {
            source, target, dataset, from_snapshot, to_snapshot, options
        }
    }
    ...
    fn insecure(self, insecure: &InsecureHost) -> Result<Transaction, TransactionBuildError> {
        ...
        zfs_send_args.extend([
            "-i".to_string(),
            format!("{}{}@{}", self.source.get_root_ref().as_str(), self.dataset, self.from_snapshot),
            format!("{}{}@{}", self.source.get_root_ref().as_str(), self.dataset, self.to_snapshot),
        ]);
        let mut send_cmds =
        ...
        Ok(Transaction::new(
            TransactionMeta::TransferIncremental(TransferIncrementalMeta {
...
impl BaseBuilder for TransferIncrementalBuilder<'_> {
