...
https://github.com/pleiszenburg/abgleich
	...
	src/abgleich/zfs/__init__.py: ZFS package root
...
def get_backup_ops(tree_a, prefix_a, tree_b, prefix_b, ignore):
    ...
    for name in tree_names:
        ...
        if dataset_in_a and not dataset_in_b:
            res.append(
                ["push_snapshot", (name, subdict_a[name]["SNAPSHOTS"][0]["NAME"])]
            )
            for snapshot_1, snapshot_2 in zip(
                subdict_a[name]["SNAPSHOTS"][:-1], subdict_a[name]["SNAPSHOTS"][1:]
            ):
                res.append(
                    [
        ...
        if source_index is None:
            raise ValueError(
                'no common snapshots in dataset "%s" - can not send incremental' % name
            )
        for snapshot_1, snapshot_2 in zip(
...
def get_tree(host=None):
    ...
    datasets = parse_table(
        run_command(cmd_list), ["NAME", "USED", "AVAIL", "REFER", "MOUNTPOINT"]
    )
    snapshots = parse_table(
        run_command(cmd_list_snapshot), ["NAME", "USED", "AVAIL", "REFER", "MOUNTPOINT"]
    )
    properties = parse_table(
        run_command(cmd_list_property), ["NAME", "PROPERTY", "VALUE", "SOURCE"]
    )
...
def push_snapshot(host, src, src_firstsnapshot, dest, debug=False):
    ...
    cmd2 = ssh_command(host, ["zfs", "receive", dest], compression=False)
    run_chain_command(cmd1, cmd2, debug=debug)
...
def push_snapshot_incremental(host, src, src_a, src_b, dest, debug=False):
    ...
    cmd1 = ["zfs", "send", "-c", "-i", "%s@%s" % (src, src_a), "%s@%s" % (src, src_b)]
    cmd2 = ssh_command(host, ["zfs", "receive", dest], compression=False)
    run_chain_command(cmd1, cmd2, debug=debug)
